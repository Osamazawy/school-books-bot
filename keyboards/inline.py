...
def get_main_menu_keyboard(is_admin: bool = False) -> InlineKeyboardMarkup:
    """القائمة الرئيسية للبوت (أزرار رئيسية علوية بعرض كامل)."""
    keyboard = [
...
def get_admin_main_keyboard() -> InlineKeyboardMarkup:
    """لوحة تحكم المشرف الرئيسية (أزرار رئيسية بعرض كامل)."""
    keyboard = [
...
def get_admin_classes_list_keyboard(classes: List[Dict[str, Any]], stage_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard.append([
        InlineKeyboardButton("🛑 حذف المرحلة", callback_data=f"adm_del_stg_confirm_{stage_id}"),
...
def get_admin_class_books_list_keyboard(books: List[Dict[str, Any]], class_id: int, stage_id: int = 1) -> InlineKeyboardMarkup:
    ...
    keyboard.append([
        InlineKeyboardButton("🔴 تفريغ الكتب", callback_data=f"adm_del_all_bks_confirm_{class_id}"),
        InlineKeyboardButton("✏️ تعديل اسم الصف", callback_data=f"adm_ren_cls_{class_id}")
    ])
    keyboard.append([InlineKeyboardButton("🛑  حذف هذا الصف بالكامل", callback_data=f"adm_del_cls_confirm_{class_id}")])
...
def get_admin_single_book_card_keyboard(book_id: int, class_id: int) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🔴 حذف الكتاب", callback_data=f"adm_del_bk_confirm_{book_id}"),
